...
func (s *StatsDaemon) GraphiteQueue(c *out.Counters, g *out.Gauges, t *out.Timers, deadline time.Time) {
	...
	file, _ := os.OpenFile("/tmp/prometheus_metrics", os.O_CREATE, 0666)
	file.Truncate(0)
	file.Seek(0,0)
...
func (s *StatsDaemon) prometheusWriter() {
	...
	defer file.Close()
        ...
        for _, line := range bytes.Split(buf, []byte("\n")) {
            ...
            if strings.HasPrefix(data[0], s.fmt.Prefix_counters) || strings.Contains(data[0], "mtype_is_counter") {
                key := strings.Replace(data[0], ".", "_", -1)
		n, _ := io.WriteString(file, fmt.Sprintf("# HELP %s autogenerated by statsdaemon\n# TYPE %s counter\n%s %s\n", key, key, key, data[1]))
		log.Debugf("Wrote %d stats to metrics file")
		...
		log.Debugf("Wrote %d stats to metrics file")
			...
			log.Debugf("Wrote %d stats to metrics file")
                    ...
                    timer_base_pos := strings.LastIndex(data[0], ".")
                    key := strings.Replace(data[0], ".", "_", -1)
		    n, _ := io.WriteString(file, fmt.Sprintf("# HELP %s autogenerated by statsdaemon\n# TYPE %s summary\n%s %s\n", data[0][0:timer_base_pos], data[0][0:timer_base_pos], key, data[1]))
		    log.Debugf("Wrote %d stats to metrics file")
                }
            }
        }
...
func (s *StatsDaemon) prometheusListener() {
	...
	s.pmb = true
	file, _ := os.Open("/tmp/prometheus_stats")
